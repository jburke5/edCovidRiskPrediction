...
baselineChartData = [{'x': mortalityDF.ageMedian, 'y': mortalityDF.caseFatality , 'type': 'scatter', 'name': 'Median Mortality for Age', 'mode' : 'markers', 'marker' : {'size':'10'}}]
baselineChartLayout =  {'title': 'Age-Mortality Relationship', 'xaxis': {'title': 'Age', 'range':[30, 90]},'yaxis': {'title': 'Mortality', 'range': [-0.10, 1.0]}}
...
app.layout = html.Div(children=[
    ...
    html.Div(children=[
    	html.Div(children=[
    		...
    		html.Div(children=[html.Label('D-Dimer'), dcc.Input(type='number', value=1.0, id='dDimerInput')], id="dDimerPanel"),
		    html.Div(children=[html.Label('Ferritin'), dcc.Input(type='number', value=50, id='ferritinInput')], id="ferritinPanel"),
		...
		html.Div(children=[
			dcc.Graph(id='chartOfMortality',
				figure={'data': baselineChartData,'layout': baselineChartLayout}
			),
	], className='column', id='figurePanel', style={'width':'50%'}),
...
@app.callback(
    ...
    Output(component_id='chartOfMortality', component_property='figure')],
...
def update_risks(age, dDimer, ferritin):
	...
	newChartData = {'x':[age], 'y':[combinedProb.nominal_value], 'type' : 'scatter', 'mode' : 'marker', 'name' : 'Predicted Mortality for Characteristics', 'marker' : {'size' : 20}}
	...
	mortalityString = f"{combinedProb.nominal_value*100:.0f}%"
	ciString = f"95% CI [{(combinedProb.nominal_value-combinedProb.std_dev*1.96)*100:.0f}% - {(combinedProb.nominal_value+combinedProb.std_dev*1.96)*100:.0f}%]"
	
	return mortalityString, ciString, figure
